<template lang="pug">
    #philosophers-question_singlechoice
        ...
        .uk-card.uk-card-default(v-if="areGeneralFeedbackAllowed")
        .uk-card-header(style="padding-top: 5px; padding-bottom: 5px;")
        b.uk-h2(v-if="areGeneralFeedbackAllowed") {{ "Hintergrund: " }}
        .uk-card-body
              p._question(v-if="areGeneralFeedbackAllowed" ,v-html="mdl_question.feedback")
              
       
                div(v-for="answer in mdl_answers", :key="answer.id", class="uk-width-1-1@s uk-width-1-2@m")
                     p._answer(v-if="areKlickedFeedbackAllowed && clickedAnswerId === answer.id" ,v-html="answer.feedback")
        .uk-card.uk-card-default
        .uk-card-header(style="padding-top: 5px; padding-bottom: 5px;")
...
//p._question(v-if="areActionsAllowed" ,v-html="mdl_question.feedback")
//span.uk-width-expand.uk-text-center(v-html="answer.feedback")
//vk-grid.uk-margin-top(matched)
//            div(v-for="answer in mdl_answers", :key="answer.id", class="uk-width-1-1@s uk-width-1-2@m")
//                    vk-grid.uk-grid-small
//                        span.uk-width-auto.uk-text-bold {{ answer.label }}
    ...
    export default {
        ...
        computed: {
            ...
            isWrongAnswerTrue() {
             ...
             areGeneralFeedbackAllowed() {
                return this.question.finished && this.game.general_feedback;
            },
            
              areKlickedFeedbackAllowed() {
                return this.question.finished && this.game.given_answer_feedback;
            }
        ...
        methods: {
            ...
            selectAnswer(answer) {
                  console.log( this.game.given_answer_feedback);
                  console.log( this.game.questions_quantity);
            ...
            getAnswerClasses(answer) {
                ...
                if (this.isFinished) {
                    ...
                    if (this.isClickedAnswer(answer)) {
                        ...
                        if (this.isWrongAnswerGiven(answer)) {
                           
                            result.push('uk-alert-danger');
                            this.setisWrongAnswer(true);
