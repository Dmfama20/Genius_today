...
import $ from 'jquery';
import { MODE_INTRO, MODE_HINT, MODE_LEVELS, MODE_QUESTION, VALID_MODES } from './constants';
...
export const store = new Vuex.Store({
    ...
    mutations: {
        ...
        setisWrongAnswer(state, isWrongAnswer) {
            state.isWrongAnswer = isWrongAnswer;
            console.error("isWrongAnser: " + state.isWrongAnswer + ".");
        },
        setSecondChance(state, secondChance) {
            state.secondChance = secondChance;
            console.error("secondChance: " + state.secondChance + ".");
        },
        setHintSeen(state, hintSeen) {
            state.hintSeen = hintSeen;
            console.error("hintSeen: " + state.hintSeen + ".");
        },
        setGame(state, game) {
            state.game = game;
        },
        ...
        incrementCountAnswers(state, countAnswers) {
            state.countAnswers++;
            console.error("CountAsnwers: " + state.countAnswers + ".");
        },
        resetCountAnswers(state, countAnswers) {
        ...
        choseChapter(state, chapter) {

            console.error("Chapter chosen before: " + chapter + ".");
    ...
    actions: {
        ...
        async showQuestionForLevel(context, levelId) {
            ...
            return context.dispatch('fetchQuestion', levelId, chapter).then(() => {
         ...
         * @returns {Promise<void>}
         */
        async fetchQuestion(context, levelId, chapter) {
            let args = {
                gamesessionid: this.state.gameSession.id,
                levelid: levelId,
