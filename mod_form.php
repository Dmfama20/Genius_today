...
class mod_philosophers_mod_form extends moodleform_mod {
    ...
    public function definition() {
        ...
        $mform->addHelpButton('question_shuffle_answers', 'question_shuffle_answers', 'philosophers');
         // ... sequetiell question ordering?
        $mform->addElement('advcheckbox', 'sequential_question_ordering', "Sequentielle Fragenauswahl an Hand Fragen-ID", '&nbsp;');
        $mform->setDefault('sequential_question_ordering', 0);
        // ... Number of questions 
          $mform->addElement('text', 'questions_quantity',"Anzahl Fragen in Fragensammlung", ['size' => 5]);
        $mform->setType('questions_quantity', PARAM_INT);
        $mform->setDefault('questions_quantity', 20);
        //$mform->addHelpButton('review_duration', 'review_duration', 'philosophers');
         // ... general feedback?
        $mform->addElement('advcheckbox', 'general_feedback', "Allgemeines Feedback", '&nbsp;');
        $mform->setDefault('general_feedback', 0);
         // ... special Feedback?
        $mform->addElement('advcheckbox', 'given_answer_feedback', "Spezielles Feedback der gegebenen Antwort", '&nbsp;');
        $mform->setDefault('given_answer_feedback', 0);
       // $mform->addHelpButton('question_shuffle_answers', 'question_shuffle_answers', 'philosophers');
        // ... highscore count
