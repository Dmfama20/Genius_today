...
class level extends abstract_model {
    ...
    public function get_categories(): array {
     ...
     /**
     * Returns one  question out of the categories that are assigned to this level and the chapter.
     *
     * @return \question_definition
     * @throws \dml_exception
     * @throws \coding_exception
     */
    public function get_chapter_question(int $chapter): \question_definition {
        global $DB;
        ...
        // build query for moodle question selection which is depending on idnumber!
        //add random question if empty!
        $sql = "
            SELECT q.id
              FROM {question} q 
        INNER JOIN {qtype_multichoice_options} qmo ON q.id=qmo.questionid
             WHERE q.qtype = ? AND qmo.single = ? AND q.category $cat_sql AND q.idnumber = $chapter
        ";
...
//             }
           ...
           $id = \reset($available_ids)->id;
            ...
            throw new \dml_exception('No question available! Make sure you have filled all fields "ID-Number" in the question-bank.');
        }
    }
    /**
