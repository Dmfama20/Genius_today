...
class gamesession extends abstract_model {
    ...
    public function is_level_finished($id_level) {
       ...
       /**
     * Deletes  the total wrong levels of the given user.
    
     */
    public function delete_wrong_levels() {
        global $DB;
        $DB->delete_records('philosophers_questions', ['gamesession' => $this->get_id(), 'correct' => false]);

    }


     /**
     * Looks up in the db, whether the given level is already finished and answered correctly
     *
     * @param int $id_level
     *
     * @return bool
     * @throws \dml_exception
     */
    public function is_level_finished_and_answered_correctly($id_level) {
        $question = $this->get_question_by_level($id_level);
        return ($question !== null && $question->is_correct());
    }
