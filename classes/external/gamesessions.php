...
class gamesessions extends external_api {
    ...
    public static function get_question_parameters() {
        return new external_function_parameters([
            ...
            'chapter' => new external_value(PARAM_INT, 'chapter given')
        ]);
    }
       /**
     * Definition of parameters for {@see get_current_question}.
     ...
     * @return external_function_parameters
     */
    public static function get_chapter_question_parameters() {
        return new external_function_parameters([
            'coursemoduleid' => new external_value(PARAM_INT, 'course module id'),
            'gamesessionid' => new external_value(PARAM_INT, 'game session id'),
            'levelid' => new external_value(PARAM_INT, 'id of the level'),
            'chapter' => new external_value(PARAM_INT, 'number of the chapter')
        ]);
...
//         ]);
     ...
     * @throws restricted_context_exception
     */
    public static function get_question_depending_on_chapter($coursemoduleid, $gamesessionid, $levelid) {
        $params = [
        ...
        self::validate_parameters(self::get_chapter_question_parameters(), $params);
...
//         self::validate_parameters(self::get_question_parameters(), $params);
        ...
        if (!$gamesession->is_level_finished($level->get_id()) && $question === null) {
            ...
            $mdl_question = $level->get_chapter_question();
...
//         global $PAGE;
//         $renderer = $PAGE->get_renderer('core');
//         $ctx = $coursemodule->context;get_question_parameters
//         $game = util::get_game($coursemodule);
//
//         // try to find existing in-progress gamesession or create a new one
//         $gamesession = util::get_or_create_gamesession($game);
...
//     }
 ...
 /**
     ...
     * @param int $coursemoduleid
     * @param int $gamesessionid
     * @param int $levelidget_question_parameters
     * @param int $chapter
     *
     * @return stdClass
     * @throws dml_exception
     ...
     * @throws restricted_context_exception
     */
    public static function get_question($coursemoduleid, $gamesessionid, $levelid, $chapter) {
        $params = [
            'coursemoduleid' => $coursemoduleid,
            'gamesessionid' => $gamesessionid,
            'levelid' => $levelid,
       ...
       self::validate_parameters(self::get_question_parameters(), $params);
        ...
        if (!$gamesession->is_level_finished($level->get_id()) && $question === null) {
            ...
            $question->set_level($level->get_id());
             ...
             if ($game->is_sequential_question_ordering()) {
                 $mdl_question = $level->get_chapter_question($chapter);
            }

            else{$mdl_question = $level->get_random_question();
            }
//
    ...
    public static function submit_answer($coursemoduleid, $gamesessionid, $questionid, $mdlanswerid) {
        ...
        if ($question->is_correct()) {
            ...
            //      $level->set_state(level::STATE_INACTIVE);
            //       $level->save();
            // NEW***************************************
...
if ($gamesession->get_answers_correct() !== $game->count_active_levels()) {
    // $gamesession->set_state(gamesession::STATE_PROGRESS);

    $gamesession->delete_wrong_levels();
    $gamesession->set_answers_total($gamesession->get_answers_correct());
    $gamesession->set_state(gamesession::STATE_PROGRESS);



    // throw new moodle_exception('THere are wrong QUestions t!!.');
